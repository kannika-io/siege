...
cf:
    ...
    @until curl -sf http://localhost:18081/subjects > /dev/null 2>&1; do sleep 1; done
    @echo "Confluent ready (Kafka: localhost:9092, Schema Registry: localhost:18081, UI: localhost:9080)"

# Build and run the API server (with seed topics)
api: cf
    cargo run -p siege-api -- --bootstrap-servers localhost:9092 --schema-registry-url http://localhost:18081 --port {{api_port}} --seed --post-seed-hook ./scripts/reset-armory/init.sh
